...
@interface CFCoverFlowView () <UIGestureRecognizerDelegate>{
    CGFloat _width;
    CGFloat _height;
...
- (void)setPageItemsWithImageNames:(NSArray *)imageNames {
    ...
    //NSArray * array = @[@"中文",@"中文",@"中文",@"中文",@"中文",@"中文"];
    for (NSInteger i = 0; i < _pageItemCount; i++) {
        ...
        UISwipeGestureRecognizer *longPressGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(longPressHandler:)];
        //longPressGesture.minimumPressDuration = 0.05;
        longPressGesture.cancelsTouchesInView = NO;
        longPressGesture.delegate = self;
        [imageView addGestureRecognizer:longPressGesture];
        
        UITapGestureRecognizer * tapPressGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(longPressHandler:)];
        tapPressGesture.delegate = self;
//        //longPressGesture.minimumPressDuration = 0.05;
        tapPressGesture.cancelsTouchesInView = NO;
        [imageView addGestureRecognizer:tapPressGesture];
        UIPanGestureRecognizer * ttapPressGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(longPressHandler:)];
        ttapPressGesture.delegate = self;
        //longPressGesture.minimumPressDuration = 0.05;
        ttapPressGesture.cancelsTouchesInView = NO;
        [imageView addGestureRecognizer:ttapPressGesture];
//
       ...
       [imageView addSubview:view];
        ...
        UIImageView * logoImg = [[UIImageView alloc]initWithFrame:CGRectMake((imageView.frame.size.width-65)/2, 27, 65, 65)];
        
        logoImg.layer.masksToBounds = YES;
        logoImg.layer.cornerRadius = 5;
        logoImg.layer.borderWidth = 1;
        logoImg.layer.borderColor = [UIColor colorWithRed:225/255.0 green:225/255.0 blue:225/255.0 alpha:1].CGColor;
        [view addSubview:logoImg];
        [_imgArray addObject:logoImg];
        UILabel * titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(logoImg.frame)+25, imageView.frame.size.width, 16)];
        ...
        UILabel * dicLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(titleLabel.frame)+11, imageView.frame.size.width, 14)];
        
        dicLabel.textColor = [UIColor colorWithRed:174/255.0 green:174/255.0 blue:174/255.0 alpha:1];
        dicLabel.font = [UIFont systemFontOfSize:14];
        dicLabel.textAlignment = NSTextAlignmentCenter;
        
        [view addSubview:dicLabel];
        
        [_dicArray addObject:dicLabel];
        
        UIView * shitView = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(dicLabel.frame)+34, imageView.frame.size.width, 0.5)];
        shitView.backgroundColor = [UIColor colorWithRed:209/255.0 green:209/255.0 blue:209/255.0 alpha:1];
        [view addSubview:shitView];
        
        UILabel * companyLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(shitView.frame), imageView.frame.size.width, imageView.frame.size.height-CGRectGetMaxY(shitView.frame))];
        companyLabel.textAlignment = NSTextAlignmentCenter;
        companyLabel.font = [UIFont systemFontOfSize:13];
        [_companyArray addObject:companyLabel];
        [view addSubview:companyLabel];
        
        if (screen_Width == 320) {
            logoImg.frame = CGRectMake((imageView.frame.size.width-65)/2, 22, 65, 65);
            titleLabel.frame = CGRectMake(0, CGRectGetMaxY(logoImg.frame)+24, imageView.frame.size.width, 16);
            dicLabel.frame = CGRectMake(0, CGRectGetMaxY(titleLabel.frame)+9, imageView.frame.size.width, 14);
            shitView.frame = CGRectMake(0, CGRectGetMaxY(dicLabel.frame)+23, imageView.frame.size.width, 0.5);
            companyLabel.frame = CGRectMake(0, CGRectGetMaxY(shitView.frame)-1, imageView.frame.size.width, imageView.frame.size.height-CGRectGetMaxY(shitView.frame));
        }
        CATransform3D transform = CATransform3DMakeTranslation(i * _pageItemSpace, 0, 0);
        imageView.layer.transform = transform;
        
        UIImageView * yuanshuImg = [[UIImageView alloc]initWithFrame:CGRectMake(imageView.frame.size.width/2-6, CGRectGetMaxY(shitView.frame), 12, 7)];
        yuanshuImg.image = [UIImage imageNamed:@"icon_sanjiao"];
        [view addSubview:yuanshuImg];
...
- (void)godetail:(UITapGestureRecognizer*)tap{
    [self.delegate coverFlowView:self didSelectPageItemAtIndex:tap.view.tag-1000];
}

- (void)addMessage:(NSArray*)NotMissArray{
    
    for (int i = 0; i<5; i++) {
        UIImageView * img = _imgArray[i];
        img.image = [UIImage imageNamed:@""];
        UILabel * titleLabel = _titleArray[i];
        titleLabel.text = @"";
        UILabel * dicLabel = _dicArray[i];
        dicLabel.text = @"";
        UILabel * company = _companyArray[i];
        company.text = @"";
        titleLabel.text = @"";
        if (i<NotMissArray.count) {
            NSString *str1;
            NSString *str2;
            NSString * strr;
            if (str2.length>0) {
                strr = [NSString stringWithFormat:@"%@ (%@)",str1,str2];
            }else{
                strr = [NSString stringWithFormat:@"%@",str1];
            }
            company.textColor = [UIColor colorWithRed:0.98 green:0.6 blue:0.37 alpha:1];
            NSInteger t = str1.length;
            NSMutableAttributedString *AttributedStr = [[NSMutableAttributedString alloc]initWithString:strr];
            [AttributedStr addAttribute:NSForegroundColorAttributeName
             
                                  value:[UIColor colorWithRed:34/255.0f green:34/255.0f blue:34/255.0f alpha:1]
             
                                  range:NSMakeRange(0, t  )];
                [AttributedStr addAttribute:NSFontAttributeName
                                  value:[UIFont systemFontOfSize:15]
                                  range:NSMakeRange(0, t )];
                company.attributedText = AttributedStr;
        }
        }
}
